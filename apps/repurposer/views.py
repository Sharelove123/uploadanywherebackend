...
class RepurposedPostViewSet(viewsets.ModelViewSet):
    ...
    def schedule(self, request, pk=None):
        ...
        post.status = RepurposedPost.Status.SCHEDULED
        post.save()
    ...
    def unschedule(self, request, pk=None):
        ...
        if post.status != RepurposedPost.Status.SCHEDULED:
            return Response({'error': 'Post is not scheduled'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def queue(self, request):
        """Get all scheduled posts ordered by scheduled time."""
        scheduled_posts = self.get_queryset().filter(
            status=RepurposedPost.Status.SCHEDULED
        ).order_by('scheduled_for')
        
        serializer = RepurposedPostSerializer(scheduled_posts, many=True)
        return Response(serializer.data)
    
    @action(detail=True, methods=['post'])
    def set_recurring(self, request, pk=None):
        """Set up recurring posting for a post."""
        post = self.get_object()
        
        pattern = request.data.get('pattern')  # daily, weekly, monthly
        time_str = request.data.get('time')    # HH:MM format
        days = request.data.get('days', [])    # [0,1,2] for Mon,Tue,Wed or [1,15] for day of month
        
        if pattern not in ['daily', 'weekly', 'monthly']:
            return Response({'error': 'Invalid pattern. Use daily, weekly, or monthly'}, status=status.HTTP_400_BAD_REQUEST)
        
        from datetime import datetime
        recurrence_time = None
        if time_str:
            try:
                recurrence_time = datetime.strptime(time_str, '%H:%M').time()
            except ValueError:
                return Response({'error': 'Invalid time format. Use HH:MM'}, status=status.HTTP_400_BAD_REQUEST)
        
        post.is_recurring = True
        post.recurrence_pattern = pattern
        post.recurrence_time = recurrence_time
        post.recurrence_days = days
        post.is_recurring_active = True
        post.status = RepurposedPost.Status.READY  # Template stays ready, copies get scheduled
        post.save()
        
        return Response({
            'message': f'Recurring {pattern} posting enabled',
            'post': RepurposedPostSerializer(post).data
        })
    
    @action(detail=True, methods=['post'])
    def stop_recurring(self, request, pk=None):
        """Stop recurring posting for a post."""
        post = self.get_object()
        
        post.is_recurring_active = False
        post.save()
        
        return Response({
            'message': 'Recurring posting stopped',
            'post': RepurposedPostSerializer(post).data
        })
